<script>
  ...
  export default {
    ...
    computed: {
      ...
      customDisplayTime() {
        if (!this.amText && !this.pmText) {
          this.cTimePickerFieldValue = this.displayTime
        } else {
          this.cTimePickerFieldValue = this.displayTime.replace(new RegExp(this.apm, 'g'), this.apmDisplayText(this.apm))
        }
      },
      inputIsEmpty() {
    ...
    watch: {
      ...
      'cTimePickerFieldValue'(newValue) {
        this.hour = newValue.split(':')[0];
        this.minute = newValue.split(':')[1];
      },
      value: {
        deep: true,
        handler() {
    ...
    methods: {
      ...
      renderHoursList() {
        const hourStart = this.hRange && this.hRange[0] ? this.hRange[0] : 0
        const hourEnd = this.hRange && this.hRange[1] ? this.hRange[1] : 24
      ...
      readStringValues(stringValue) {
        ...
        if (comboReg.test(stringValue)) {
          this.cTimePickerFieldValue = stringValue
...
<div class="vue__time-picker time-picker" :style="inputWidthStyle" >
  ...
  <div class="dropdown" v-show="showDropdown" :style="inputWidthStyle" tabindex="-1" @mouseup="keepFocusing"
       @click.stop="">
        ...
        <ul class="hours" @scroll="keepFocusing">
          ...
          <template v-for="(hr, hIndex) in hours">
            <li v-if="!opts.hideDisabledHours || (opts.hideDisabledHours && !isDisabledHour(hr))"
                ...
                v-text="hr"
                @click="select('hour', hr); customDisplayTime;"></li>
          </template>
        </ul>
        <ul class="minutes" @scroll="keepFocusing">
          ...
          <template v-for="(m, mIndex) in minutes">
            <li v-if="!opts.hideDisabledMinutes || (opts.hideDisabledMinutes && !isDisabledMinute(m))"
                ...
                v-text="m"
                @click="select('minute', m); customDisplayTime;"></li>
          </template>
        </ul>
